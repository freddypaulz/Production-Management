...
router.post('/wastage-return', (req, res) => {
   stocks.find({}).then((stock) => {
      for (let i = 0; i < stock.length; i++) {
         reqDetails
            .find({ _id: stock[i].Purchase_Id })
            .then((purchaseDetails) => {
               if (
                  req.body.Raw_Material_Code ===
                  purchaseDetails[0].Raw_Material_Code
               ) {
                  let new_stock =
                     stock[i].Total_Quantity + req.body.Total_Quantity;
                  stocks
                     .findByIdAndUpdate(
                        {
                           _id: stock[i]._id,
                     ...
                     .then((res) => {
                        i = stock.length;
                        res.send('Stock returned successfully');
                     });
               } else {
                  if (i === stock.length - 1) {
   ...
   console.log('Wastage stock returned successfully');
});
