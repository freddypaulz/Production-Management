...
router.post("/add", (req, res) => {
  ...
  new_Pre_Production.save().then(Pre_Production => {
    ...
    console.log(Raw_Material_Details[0].material_code);
    Production_Raw_Material_Stock.find({}).then(stock_entry => {
      for (let i = 0; i < stock_entry.length; i++) {
        for (let j = 0; j < Raw_Material_Details.length; j++) {
          if (
            stock_entry[i].Raw_Material_Code ===
            Raw_Material_Details[j].material_code
          ) {
            console.log("rmstoc:", Quantity);
            let new_stock =
              stock_entry[i].Quantity - Raw_Material_Details[j].quantity;
            Production_Raw_Material_Stock.findOneAndUpdate(
              { Raw_Material_Code: Raw_Material_Details[j].material_code },
              {
                $set: {
                  Quantity: new_stock
                }
            ...
            console.log(Raw_Material_Details[j].material_code);
